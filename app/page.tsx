...
type RetrievalInfo = { findings:number; sessions:number; tools:Array<{name:string;query:string;results:number}> };
type ChatEntry = { role: "user" | "assistant"; content: string; retrieval?:RetrievalInfo; citationsValid?:boolean; invalidCitations?:string[] };
...
export default function Home() {
  ...
  function renderAgentText(content: string, invalidCitations: string[] = []) {
    return content.split(/(\[E#\d+\]|\[S#\d+\])/g).map((part, index) => {
      if (invalidCitations.includes(part)) return <span className="invalid-citation" title={t.invalidCitationTitle} key={`${part}-${index}`}>{part}</span>;
  ...
  async function runAgent(question?: string) {
    const prompt = question?.trim() || (language === "zh" ? "生成首次调查报告，包括攻击会话、关键证据、可信度、限制和下一步建议。" : "Generate an initial investigation report covering attack sessions, key evidence, confidence, limitations, and recommended next steps.");
    ...
    try {
      ...
      const data = await response.json() as { analysis?: string; error?: string; retrieval?:RetrievalInfo; citationsValid?:boolean; invalidCitations?:string[] };
      if (!response.ok || !data.analysis) throw new Error(data.error ?? "AI analysis failed.");
      setAgentMessages((current) => [...current, { role: "assistant", content: data.analysis!, retrieval:data.retrieval, citationsValid:data.citationsValid, invalidCitations:data.invalidCitations }]);
    } catch (error) {
    ...
    <section className="dashboard" aria-live="polite">
      ...
      {!content&&!pcapResult ? <div className="empty"><div className="empty-mark">⌁</div><h2>{t.start}</h2><p>{t.empty} 也可上传 `.pcap` / `.pcapng` 抓包文件。</p></div> : <>
        ...
        <article className="panel agent">
          <div><span className="label">CONVERSATIONAL INVESTIGATION AGENT</span><h2>{t.agentTitle}</h2><p className="muted">{t.agentDescription}</p></div>
          <div className="agent-controls"><select aria-label={t.providerLabel} value={provider} onChange={(event) => setProvider(event.target.value as Provider)}><option value="openai">OpenAI</option><option value="deepseek">DeepSeek</option><option value="qwen">{language === "zh" ? "通义千问" : "Qwen"}</option><option value="kimi">Kimi</option><option value="custom">{t.customProvider}</option></select><button className="button primary" disabled={agentLoading || !result.findings.length} onClick={()=>runAgent()}>{agentLoading ? t.agentWorking : agentMessages.length ? t.agentRerun : t.agentRun}</button></div>
          <details className="api-config"><summary>{t.apiSummary}</summary><div className="api-fields"><label>API Key<input type="password" value={apiConfigs[provider].apiKey} autoComplete="off" spellCheck={false} onChange={(event) => updateApiConfig("apiKey", event.target.value)} placeholder="sk-…" /></label><label>Base URL<input type="url" value={apiConfigs[provider].baseUrl} spellCheck={false} onChange={(event) => updateApiConfig("baseUrl", event.target.value)} placeholder="https://api.example.com/v1" /></label><label>{t.modelName}<input type="text" value={apiConfigs[provider].model} spellCheck={false} onChange={(event) => updateApiConfig("model", event.target.value)} placeholder="model-name" /></label></div><p className="config-note">{t.apiNote}</p></details>
          {agentMessages.length>0&&<div className="agent-transcript" aria-live="polite">{agentMessages.map((message,index)=><div className={`chat-message ${message.role}`} key={`${message.role}-${index}`}><span>{message.role==="assistant"?"AGENT":"YOU"}</span><div>{message.role==="assistant"&&message.retrieval&&<div className="retrieval-strip"><strong>{t.retrieval}</strong><span>{message.retrieval.findings} {t.evidenceCount}</span><span>{message.retrieval.sessions} {t.sessionCount}</span>{message.retrieval.tools.map((tool,toolIndex)=><span className="tool-chip" title={tool.query} key={`${tool.name}-${toolIndex}`}>{tool.name} · {tool.results}</span>)}</div>}{message.role==="assistant"&&message.citationsValid===false&&<div className="citation-warning">{t.citationWarning}: {message.invalidCitations?.join(language === "zh" ? "、" : ", ")} {t.citationMissing}</div>}<div className="chat-content">{message.role==="assistant"?renderAgentText(message.content,message.invalidCitations):message.content}</div></div></div>)}{agentLoading&&<div className="chat-message assistant pending"><span>AGENT</span><div>{t.agentPending}</div></div>}</div>}
          <form className="agent-question" onSubmit={(event)=>{event.preventDefault();if(agentQuestion.trim()&&!agentLoading)runAgent(agentQuestion);}}><label htmlFor="agent-question">{t.followUp}</label><div><input id="agent-question" value={agentQuestion} onChange={(event)=>setAgentQuestion(event.target.value)} disabled={agentLoading||!result.findings.length} placeholder={t.followUpPlaceholder}/><button className="button primary" disabled={agentLoading||!agentQuestion.trim()||!result.findings.length} type="submit">{t.send}</button></div></form>
