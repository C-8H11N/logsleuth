...
export type PcapResult = { packets:number; findings:PcapFinding[] };
...
export function parsePcapng(buffer:ArrayBuffer):PcapResult {
  ...
  const links:number[]=[],syn=new Map<string,Set<number>>(),dns=new Map<string,number>(),findings:PcapFinding[]=[]; let packets=0,offset=0;
